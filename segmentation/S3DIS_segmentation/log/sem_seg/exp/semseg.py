...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        ...
        # 编码器前向传播
        local_feat = self.first_conv(point_groups.transpose(2, 1))  # BG 256 N

        # 获取全局特征并扩展
        feature_global = torch.max(local_feat, dim=2, keepdim=True)[0]  # BG 256 1 - 最大池化

        # 拼接全局和局部特征
        fusion_feat = torch.cat([feature_global.expand(-1, -1, n), local_feat], dim=1)  # BG 512 N

        # 第二层编码，添加残差连接
        res = self.res_proj(local_feat)  # 投影残差连接 BG C N
        out_feat = self.second_conv(fusion_feat) + res  # 残差连接
        out_feat = self.relu(out_feat)  # 应用激活函数

        # 再次全局池化获得每个组的特征表示，结合平均池化和最大池化
        max_feat = torch.max(out_feat, dim=2, keepdim=False)[0]  # BG C
        avg_feat = torch.mean(out_feat, dim=2)  # BG C
        feature_global = max_feat + avg_feat  # 特征融合
...
class TransformerEncoder(nn.Module):
    '''
    Transformer编码器，包含多个编码器块，支持多层特征提取
    '''
    ...
    def forward(self, x, pos):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in fetch_idx:
                feature_list.append(self.norm(x))  # 收集特定层的归一化特征

        return feature_list


class MultiScaleFeaturePropagation(nn.Module):
    """
    多尺度特征传播模块，用于将特征上采样回原始点云，同时融合多个尺度的特征
    """

    def __init__(self, in_channel, mlp, mid_scales=[0.25, 0.5]):
        super(MultiScaleFeaturePropagation, self).__init__()
        self.mid_scales = mid_scales  # 中间尺度比例
        ...
        # 创建中间尺度的MLP层
        self.mid_mlps = nn.ModuleList()
        for _ in range(len(mid_scales)):
            self.mid_mlps.append(nn.Sequential(
                nn.Conv1d(in_channel, mlp[0] // 2, 1),
                nn.BatchNorm1d(mlp[0] // 2),
                nn.ReLU(inplace=True)
            ))

        # 融合所有尺度的特征
        self.fusion_mlp = nn.Sequential(
            nn.Conv1d(mlp[0] + mlp[0] // 2 * len(mid_scales), mlp[-1], 1),
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        xyz1: (B, N, 3) 低层次的点云坐标
        xyz2: (B, S, 3) 高层次的点云坐标
        points1: (B, C1, N) 低层次的点云特征
        points2: (B, C2, S) 高层次的点云特征
        """
        B, N, C = xyz1.shape
        _, S, _ = xyz2.shape
        ...
        # 如果低层没有特征，则初始化为0
        if points1 is None:
            points1 = xyz1.transpose(1, 2).contiguous()
        ...
        # 处理主路径的特征
        main_features = interpolated_points.clone()
        for i, conv in enumerate(self.mlp_convs):
            main_features = conv(main_features)

        # 处理中间尺度的特征
        mid_features = []
        for i, scale in enumerate(self.mid_scales):
            mid_points_num = max(int(S * scale), 1)
            # 采样出中间尺度的点
            fps_idx = pointnet2_utils.furthest_point_sample(xyz2, mid_points_num)
            mid_xyz = pointnet2_utils.gather_operation(xyz2.transpose(1, 2).contiguous(), fps_idx).transpose(1,
                                                                                                             2).contiguous()
            mid_features_sampled = pointnet2_utils.gather_operation(points2, fps_idx)

            # 为中间尺度点进行特征传播
            dist, idx = pointnet2_utils.three_nn(xyz1, mid_xyz)
            dist_recip = 1.0 / (dist + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdim=True)
            weight = dist_recip / norm

            interpolated_mid = pointnet2_utils.three_interpolate(mid_features_sampled, idx, weight)
            interpolated_mid = self.mid_mlps[i](interpolated_mid)
            mid_features.append(interpolated_mid)

        # 融合所有尺度的特征
        all_features = [main_features] + mid_features
...
class get_model(nn.Module):
    ...
    def __init__(self, cls_dim):
        ...
        # TODO分组参数 - 增加组数量和每组点数 尝试32  128
        self.group_size = 48  # 每组点数，增加以获取更多局部信息
        self.num_group = 384  # 组数量，增加以覆盖更多点云区域
        ...
        # 使用增强的多尺度特征传播层，用于将特征上采样回原始点云
        self.propagation_0 = MultiScaleFeaturePropagation(
            in_channel=1152 + 3,  # 3个Transformer特征层拼接(384*3)加上坐标
            mlp=[self.trans_dim * 4, 1024],
            mid_scales=[0.25, 0.5]  # 添加中间尺度
        )

        # 分类头，使用更强的特征融合和减少通道数压缩
    ...
    def forward(self, pts):
        ...
        # 转换输入特征
        input_features = self.input_trans(features)  # B 256 N
        ...
        # 归一化并调整维度
        feature_list = [feat.transpose(-1, -2).contiguous() for feat in feature_list]  # B C G
