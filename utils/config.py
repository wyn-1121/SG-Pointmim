...
def cfg_from_yaml_file(cfg_file):
    config = EasyDict()
    print_log(f"Loading config from {cfg_file}", logger=None)  # Debugging line
    with open(cfg_file, 'r') as f:
        try:
            new_config = yaml.load(f, Loader=yaml.FullLoader)
        except:
            new_config = yaml.load(f)
    print_log(f"Loaded new_config: {new_config}", logger=None)  # Debugging line
...
def get_config(args, logger=None):
    ...
    # Debugging: print config to ensure it's loaded correctly
    print_log(f"Loaded config: {config}", logger=logger)
